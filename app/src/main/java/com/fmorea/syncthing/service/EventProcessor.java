...
public class EventProcessor implements  Runnable, RestApi.OnReceiveEventListener {
    ...
    private void onPendingDevicesChanged(Map<String, String> added) {
        ...
        Log.d(TAG, "Magic: Automatically accepting device '" + deviceName + "' (" + deviceId + ")");
        ...
        // Stabilize mesh: Don't set as introducer automatically in background handshake
        // to avoid potential gossip loops unless explicitly added by user.
        device.introducer = false;
        
        mRestApi.updateDevice(device);
        
        // Also share LinkThing folder
        Folder folder = mRestApi.getFolderByID(Constants.LINKTHING_FOLDER_ID);
        if (folder != null) {
            folder.addDevice(device);
            mRestApi.updateFolder(folder);
        }
    }

    private void onPendingFoldersChanged(Map<String, Object> added) {
        String deviceId = (String) added.get("deviceID");
        String folderId = (String) added.get("folderID");
        String folderLabel = (String) added.get("folderLabel");
        if (deviceId == null || folderId == null) {
            return;
        }

        // Magic: Only auto-accept if it's the LinkThing folder
        if (Constants.LINKTHING_FOLDER_ID.equals(folderId)) {
            Log.d(TAG, "Magic: Automatically accepting LinkThing folder from '" + deviceId + "'");
            
            Folder folder = mRestApi.getFolderByID(folderId);
            if (folder == null) {
                folder = new Folder();
                folder.id = folderId;
            ...
            // Add the sharing device to the folder
            Device sharingDevice = null;
            ...
            if (sharingDevice != null) {
                folder.addDevice(sharingDevice);
            }
            
            mRestApi.updateFolder(folder);
        } else {
            // Fallback for other folders (still show notification)
            Log.d(TAG, "Device '" + deviceId + "' wants to share unknown folder '" +
            ...
            mNotificationHandler.showFolderShareNotification(
                ...
                folderId,
                folderLabel,
                (Boolean) added.get("receiveEncrypted"),
                (Boolean) added.get("remoteEncrypted"),
                isNewFolder
            );
        }
    }
